...
class OmniBase():
	...
	max_speed_linear  = 1.3
	max_speed_angular = 7.0
	...
	log_file = open("/home/ropod/Desktop/Output.log", "w")
	log_file.close()
	log_file = open("/home/ropod/Desktop/Output.log", "w")
	...
	def __init__(self):
		...
		while not time_cnt > 8.5:#rospy.is_shutdown():
			just = now
			now = rospy.Time.now()
			dt = (now - just).to_sec()
			...
			if self.vel and ref_t + ref_dt < now:
				...
				#self.vel = None
				ref_t = now
				print 'time_cnt: %f ' % (time_cnt)
				# Simulated velocity
				if time_cnt <8.0:				
					#self.vel.linear.x = self.vel.linear.x + 0.15/ref_freq; # constant acceleraton
					self.vel.angular.z = self.vel.angular.z + 1.0/ref_freq; # constant acceleraton
				else:
					self.vel.linear.x = 0 # stop
					# self.vel.angular.z = 0 # stop
			...
			if result and dt > 0.0:
				...
				vel_rightfront = float(result.group(1))
				V_rightfront = float(result.group(2))
					
				I_rightfront = float(result.group(3))
				I_back       = float(result.group(4))
				I_leftfront  = float(result.group(5))
				...
				# print 'Voltages: %f %f %f' % (V_rightfront, V_back, V_leftfront)
				# print 'Currents: %f %f %f' % (I_rightfront, I_back, I_leftfront)
				# print 'Vel vx %f ' % (vx)
				#print odom
				odomPub.publish(odom)
				
				self.log_file.write('%.3f\t%.3f\t%.3f\t%.3f\n' % (time_cnt , vel_rightfront, V_rightfront, I_rightfront))
				# self.log_file.write('%.3f\t%.3f\t\n' % (dt , vx))
				self.log_file.flush()
