...
class OmniBase():
	...
	def __init__(self):
		...
		odomPub =rospy.Publisher(''.join([rospy.get_namespace(), 'odom']), Odometry, queue_size=0) #queue_size: This is the size of the outgoing message queue used for asynchronous publishing. Please find more detailed information below in the section "Choosing a good queue_size" 
		currentPub = rospy.Publisher(''.join([rospy.get_namespace(), 'current']), motorCurrent, queue_size=0) #queue_size: This
		...
		while not rospy.is_shutdown():
			...
			if result and dt > 0.0:
				...
				odomBroadcaster.sendTransform(
					...
					rospy.get_namespace() + "odom"
					)
			
				odom = Odometry()
				odom.header.frame_id = rospy.get_namespace() + "odom"
				odom.child_frame_id = rospy.get_namespace() + "base_link"
				...
				odom.header.frame_id = rospy.get_namespace() + "current"
