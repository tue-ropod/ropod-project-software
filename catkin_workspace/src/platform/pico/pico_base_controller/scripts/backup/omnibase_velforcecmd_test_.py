...
class OmniBase():
	...
	def __init__(self):
		...
		while not time_cnt > 6.5:#rospy.is_shutdown():
			just = now
			now = rospy.Time.now()
			dt = (now - just).to_sec()
			...
			if  (self.vel or self.force) and ref_t + ref_dt < now: # 
				...
				# Simulated force
				if time_cnt < 6.0:
					self.cntrmode.data = 1
					# self.force.linear.x = 35 # constant force
					self.force.angular.z = 20 # constant force
					# self.cntrmode.data = 0
					# self.vel.linear.x = self.vel.linear.x + 0.15/ref_freq # constant acceleraton
				else:
					self.force.linear.x = 0 # stop
			...
			if result and dt > 0.0:
				...
				self.log_file.write('%.3f\t%.3f\t%.3f\t%.3f\t%.3f\n' % (time_cnt , vx, I_rightfront, I_back, I_leftfront))
				self.log_file.flush()
