		...
		while not time_cnt > 8.5:#rospy.is_shutdown():
			...
			if self.vel and ref_t + ref_dt < now:
				...
				if time_cnt <8.0:				
					...
					# self.vel.angular.z = self.vel.angular.z + 0.5/ref_freq; # constant acceleraton
				else:
			...
			if result and dt > 0.0:
				...
				enc1 = float(result.group(4))
				enc2 = float(result.group(5))
				enc3 = float(result.group(6))
				...
				currentMsg.I_rightfront = 0
				currentMsg.I_back = 0
				currentMsg.I_leftfront = 0
